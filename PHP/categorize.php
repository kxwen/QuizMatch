...
function writeUser_results($passedInQuiz){
    ...
    if(mysqli_affected_rows($link) > 0){
        $sqluser_results = "UPDATE user_results SET result_id =".$resultID." WHERE user_id = ".$profile['id']." AND quiz_id = ".$passedInQuiz;
        /*$stmt = mysqli_prepare($link, $sqluser_results);
        $user_results = mysqli_stmt_bind_param($stmt, "s", $param_resultid);
